...
ggplot2::ggplot(cerca, ggplot2::aes(long, lat, group = group)) +
        ...
        ggplot2::labs(title = "Covid-19 Incidencia Acumulada 14 días",
                      ...
                      caption = "Datos: @escovid19data | Geometría: IGN | Elaboración: @JKniffki - KStats®") 
...
datos %>% 
        ...
        ggplot2::coord_flip() +
        ggplot2::scale_fill_fermenter(palette = "Blues", direction = 1) +
        ggplot2::geom_text(ggplot2::aes(label = deceassed_per_100000), hjust = -0.05, size = 3) +
        ggplot2::theme(legend.key.size = ggplot2::unit(0.9,"line"),
                       legend.key.width = ggplot2::unit(2, "cm" ),
                       legend.position = "bottom",
                       plot.background = ggplot2::element_rect(fill =   "#e6e6e1", color = NA), 
                       panel.background = ggplot2::element_rect(fill =  "#e6e6e1", color = NA), 
...
p5 <- p3 | p4  
p5 <- p5 + 
        plot_annotation(title = "Mortalidad COVID-19 (Fallecidos por 100 mil habitantes)",
